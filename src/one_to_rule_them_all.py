...
class RobotController:
    ...
    def detect_red(self, img):
        ...
        if area < self.min_red_area: return False
        M = cv2.moments(c)
        if M['m00']==0: return False
        cx = int(M['m10']/M['m00'])
        off = abs(cx - img.shape[1]//2)
        return (off < self.red_center_th)
    ...
    def detect_yellow_and_set_target(self, img):
        ...
        c = max(cnts, key=cv2.contourArea)
        area = cv2.contourArea(c)
        if area < self.min_yellow_area: return False
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.state == 'LINE':
                if img is not None and self.detect_red(img):
                    rospy.loginfo("→ RED detected, grabbing")
                    self.cmd_vel_pub.publish(Twist())
                    self.state = 'GRAB'
                    continue
                if img is not None:
                    self.line_follow(img)
                ...
                if self.current_pose and self.target_pose:
                    ...
                    d_ang        = (ang_to_goal - yaw + math.pi)%(2*math.pi) - math.pi
                    if dist <= self.stop_dist:
                        rospy.loginfo("Reached drop zone, returning home")
                        self.cmd_vel_pub.publish(Twist())
                        self.state = 'RETURN'
                    else:
                        cmd = Twist()
                        # slow down near target
                        maxs, mins, radius = 0.15, 0.02, 0.3
                        speed = mins + (maxs-mins)*min(dist/radius,1.0)
                        if abs(d_ang)>0.05:
                            cmd.angular.z = 0.4 if d_ang>0 else -0.4
                        else:
                            cmd.linear.x  = speed
                        self.cmd_vel_pub.publish(cmd)
                ...
                if self.current_pose and self.start_pose:
                    x,y,yaw     = self.current_pose
                    sx,sy,yaw0  = self.start_pose
                    dx,dy       = sx-x, sy-y
                    dist        = math.hypot(dx,dy)
                    ang_to_home = math.atan2(dy,dx)
                    d_ang       = (ang_to_home - yaw + math.pi)%(2*math.pi) - math.pi
                    cmd = Twist()
                    if dist > self.stop_dist:
                        if abs(d_ang)>0.05:
                            cmd.angular.z = 0.3 if d_ang>0 else -0.3
                        else:
                            cmd.linear.x  = 0.1
                    else:
                        yaw_diff = (yaw0 - yaw + math.pi)%(2*math.pi) - math.pi
                        if abs(yaw_diff)>0.05:
                            cmd.angular.z = 0.3 if yaw_diff>0 else -0.3
                        else:
                            rospy.loginfo("Back on line → LINE state")
                            cmd = Twist()
                            self.state = 'LINE'
                    self.cmd_vel_pub.publish(cmd)
